...
void draw_image_blackground(t_fdf * fdf)
{
    ...
    mlx_string_put(fdf->mlx_ptr, fdf->mlx_ptr, 15, 15, MENU_COLOR, "SHOW HELP MENU : press H key");
}
...
void draw_image_menu(t_fdf *fdf)
{
    ...
    mlx_string_put(fdf->mlx_ptr, fdf->mlx_ptr, 57, y += 35, MENU_COLOR,  "Show help menu      : H");
...
{
    pixel.pixel_x = ctler->camera_zoom;
    pixel.pixel_y = ctler->camera_zoom;
    pixel.pixel_z = ctler->camera_zoom / ctler->camera_z_divisor;

    pixel.pixel_x = pixel.pixel_x - image_map->image_width * ctler->camera_zoom / 2;
    pixel.pixel_y = pixel.pixel_x - image_map->image_height * ctler->camera_zoom / 2;
...
void    draw_image(t_image_map *image_map, t_fdf *fdf, t_ctler *ctrler)
{
    ...
    if (ctrler->show_menu == 1)
        draw_image_menu(fdf);
    pixel_y = 0;
    while(pixel_y < image_map->image_height)
    {
        pixel_x = 0;
        while(pixel_x < image_map->image_width)
        {
            if (pixel_x != image_map->image_width - 1)
				draw_line(project(new_point(pixel_x, pixel_y, image_map), fdf, ctrler, image_map), project(new_point(pixel_x + 1, pixel_y, image_map), fdf, ctrler, image_map), fdf);

			if (pixel_y != image_map->image_height - 1)
				draw_line(project(new_point(pixel_x, pixel_y, image_map), fdf, ctrler, image_map), project(new_point(pixel_x, pixel_y + 1, image_map), fdf, ctrler, image_map), fdf);
