...
void			get_image_map(t_raw_dot *raw_dot_list, t_image_map **image_map, int *row, int *column)
{
	...
	*image_map = (t_image_map *)malloc(sizeof(t_image_map));
	...
	while (tmp_list != NULL)
	{
		if (tmp_list->coord_z >= tmp_z_max)
			tmp_z_max = tmp_list->coord_z;
		if (tmp_list->coord_z <= tmp_z_min)
			tmp_z_min = tmp_list->coord_z;
	...
	(*image_map)->z_max = tmp_z_max;
	(*image_map)->z_min = tmp_z_min;
}
