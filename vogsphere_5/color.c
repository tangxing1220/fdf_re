...
int		get_color(t_pixel current, t_pixel str, t_pixel end)
{
	...
	diff.pixel_x = (end.pixel_x - str.pixel_x > 0\
			) ? (end.pixel_x - str.pixel_x) : (str.pixel_x - end.pixel_x);
	diff.pixel_y = (end.pixel_y - str.pixel_y > 0\
			) ? (end.pixel_y - str.pixel_y) : (str.pixel_y - end.pixel_y);
	...
	red = get_light((str.pixel_color >> 16) & 0xFF, \
			(end.pixel_color >> 16) & 0xFF, precent);
	green = get_light((str.pixel_color >> 8) & 0xFF, \
			(end.pixel_color >> 8) & 0xFF, precent);
